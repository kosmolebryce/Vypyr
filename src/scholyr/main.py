...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget,
                             ...
                             QInputDialog, QDialog, QDialogButtonBox)  # Added QInputDialog
from PyQt5.QtCore import Qt, QCoreApplication
from PyQt5.QtGui import QColor, QPalette

class Managyr:
    def __init__(self, record_file='record.json',
    ...
    def remove_class(self, course_code, section, semester):
            ...
            self.schedule = [
                cls for cls in self.schedule
                if not (cls['course_code'] == course_code and cls['section'] == section and cls['semester'] == semester)
            ]
            self.save_schedule()
    ...
    def get_gradebook(self, course_title, semester):
            gradebook_path = os.path.join(self.gradebook_dir, f"{course_title}_{semester}.json")
            if os.path.exists(gradebook_path):
    ...
    def update_class(self, original_course_code, original_section, original_semester, updated_info):
            # Find the class and update it
            for i, cls in enumerate(self.schedule):
                if (cls['course_code'] == original_course_code and
                    cls['section'] == original_section and
                    cls['semester'] == original_semester):
...
class StyledInputDialog(QInputDialog):
    ...
    @staticmethod
    def getDouble(parent, title, label, value=0.0, min=-2147483647, max=2147483647, decimals=2):
...
class ManagyrApp(QMainWindow):
    ...
    def adjustComboBoxView(self):
        ...
        for comboBox in [self.semesterComboBox, self.gradebookSemesterComboBox]:
            ...
            view.setStyleSheet("QListView::item { min-height: 20px; }")  # Set minimum height for items    
    ...
    def initScheduleTab(self):
        ...
        self.semesterComboBox.currentIndexChanged.connect(self.update_schedule_list_based_on_semester)
        ...
        self.scheduleList.currentItemChanged.connect(self.populate_fields_from_selection)
    ...
    def initGradebookTab(self):
        ...
        self.gradebookSemesterComboBox.currentIndexChanged.connect(self.populate_gradebook_list_based_on_semester)
        ...
        self.gradebookList.currentItemChanged.connect(self.populate_gradebook_from_selection)
        layout.addWidget(self.gradebookList)
    
        self.assignmentsTable = QTableWidget(0, 5)
        self.assignmentsTable.setHorizontalHeaderLabels(["Name", "Points Possible", "Points Actual", "Grade (%)", "Letter Grade"])
        self.assignmentsTable.horizontalHeader().setStretchLastSection(True)
        self.assignmentsTable.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
    ...
    def update_summary(self):
        ...
        self.totalCreditHoursLabel.setText(f"Total Credit Hours: {total_credit_hours:.1f}")
    ...
    def populate_fields_from_selection(self, current, previous):
        ...
        self.startTimeEntry.setText(info.get('start_time', ''))  # Updated to use start_time
        ...
        self.semesterEntry.setText(info.get('semester', ''))  # NEW: Populate semester field
        self.creditHoursEntry.setText(str(info.get('credit_hours', '')))  # Populate credit hours
    ...
    def update_preview(self):
        ...
        if record:
            record_str = (f"Name:         {record.get('name', 'N/A')}\n"
    ...
    def update_semester_combobox(self):
        current_semesters = {self.semesterComboBox.itemText(i) for i in range(self.semesterComboBox.count())}
    ...
    def add_class(self):
        try:
            ...
            QMessageBox.critical(self, "Invalid Input", "Credit Hours must be a numeric value.")
            return
        ...
        if not class_info['semester']:
            QMessageBox.critical(self, "Error", "Please enter the semester for the class.")
            return
    
        if any(cls['course_code'] == class_info['course_code'] and
               cls['section'] == class_info['section'] and
               cls['semester'] == class_info['semester'] for cls in self.manager.get_schedule()):
            QMessageBox.information(self, "Duplicate", "This class has already been added.")
            return
    ...
    def update_class(self):
        ...
        if not all(updated_info[key] for key in ['course_code', 'section', 'semester']):
            QMessageBox.critical(self, "Error", "Course Code, Section, and Semester are required.")
            return
    
        # If identifiers are changed, make sure it doesn't conflict with existing classes
        if ((original_course_code != updated_info['course_code'] or
             original_section != updated_info['section'] or
             original_semester != updated_info['semester']) and
            any(cls['course_code'] == updated_info['course_code'] and
                cls['section'] == updated_info['section'] and
                cls['semester'] == updated_info['semester']
                for cls in self.manager.get_schedule())):
            QMessageBox.critical(self, "Error", "Another class with the same Course Code, Section, and Semester already exists.")
            return
    
        # Proceed with the update
        self.manager.update_class(original_course_code, original_section, original_semester, updated_info)
        self.populate_semester_combobox()
        self.update_schedule_list_based_on_semester()
    ...
    def remove_class(self):
        ...
        remaining_classes_in_semester = any(cls for cls in self.manager.get_schedule() if cls['semester'] == semester)
    ...
    def update_schedule_list(self):
        ...
        for cls in self.manager.get_schedule():
            item = QListWidgetItem(f"{cls['course_code']} - {cls['section']} - {cls['course_title']}")
    ...
    def populate_gradebook_semester_combobox(self):
        current_semesters = {self.gradebookSemesterComboBox.itemText(i) for i in range(self.gradebookSemesterComboBox.count())}
        new_semesters = {cls['semester'] for cls in self.manager.get_schedule()}
    ...
    def populate_assignments_table(self, gradebook, course_title, semester):
        ...
        for i, assignment in enumerate(gradebook):
            ...
            points_possible_item = QTableWidgetItem(str(assignment.get('points_possible', 0)))
            points_actual_item = QTableWidgetItem(str(assignment.get('points_actual', 0)))
            grade_percent = (assignment.get('points_actual', 0) / assignment.get('points_possible', 0) * 100) if assignment.get('points_possible', 0) else 0
            grade_percent_item = QTableWidgetItem(f"{grade_percent:.2f}%")
            letter_grade_item = QTableWidgetItem(self.convert_percentage_to_letter_grade(grade_percent))
        ...
        overall_grade = (total_actual / total_possible * 100) if total_possible > 0 else 0
        ...
        self.assignmentsTable.setItem(overall_row, 1, QTableWidgetItem(str(total_possible)))
        self.assignmentsTable.setItem(overall_row, 2, QTableWidgetItem(str(total_actual)))
        self.assignmentsTable.setItem(overall_row, 3, QTableWidgetItem(f"{overall_grade:.2f}%"))
        self.assignmentsTable.setItem(overall_row, 4, QTableWidgetItem(overall_letter_grade))
    ...
    def add_assignment(self):
        ...
        if '(' in text and text.endswith(')'):
            ...
            QMessageBox.critical(self, "Error", "Failed to parse course title and semester.")
            return
    
        assignment_name, ok = QInputDialog.getText(self, "Add Assignment", "Enter assignment name:")
        if ok and assignment_name:
            points_possible, ok = StyledInputDialog.getDouble(self, "Add Assignment", "Enter points possible for the assignment:", decimals=2)
            if ok:
                points_actual, ok = StyledInputDialog.getDouble(self, "Add Assignment", "Enter points actual for the assignment:", decimals=2)
                if ok:
                    ...
                    gradebook.append({
                        ...
                        'grade_percent': (points_actual / points_possible * 100) if points_possible > 0 else 0
                    })
    ...
    def remove_assignment(self):
        current = self.gradebookList.currentItem()
        if not current:
            QMessageBox.critical(self, "Error", "No course selected to remove an assignment from.")
            return
        ...
        # Confirm removal
        reply = QMessageBox.question(self, "Remove Assignment", "Are you sure you want to remove this assignment?",
...
if __name__ == "__main__":
    ...
    QCoreApplication.setAttribute(Qt.AA_EnableHighDpiScaling, True)  # This should be set before creating the QApplication
