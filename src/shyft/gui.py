...
class TimerWindow:
    def __init__(self, master, time_color="#A78C7B", bg_color="#FFBE98"):
        ...
        self.start_button = tk.Button(button_frame, text="Start", command=self.start, bg=self.bg_color, highlightbackground=self.bg_color, highlightthickness=0, bd=0, font=button_font)
        self.start_button.grid(row=0, column=0, sticky="ew", padx=2)

        self.stop_button = tk.Button(button_frame, text="Stop", command=self.stop, bg=self.bg_color, highlightbackground=self.bg_color, highlightthickness=0, bd=0, font=button_font)
        self.stop_button.grid(row=0, column=1, sticky="ew", padx=2)

        self.reset_button = tk.Button(button_frame, text="Reset", command=self.reset, bg=self.bg_color, highlightbackground=self.bg_color, highlightthickness=0, bd=0, font=button_font)
...
class ShyftGUI:
    ...
    def update_styles(self):
        foreground = "white" if self.is_dark_mode else "black"
        background = "black" if self.is_dark_mode else "light gray"
        button_foreground = "white" if self.is_dark_mode else "black"
        button_background = "gray" if self.is_dark_mode else "light gray"
    ...
    def create_widgets(self):
        ...
        # Add settings button
        ttk.Button(button_frame, text="Settings", command=self.open_settings).pack(side="left", expand=True)

        # Add dark mode toggle button
        ttk.Button(button_frame, text="Toggle Dark Mode", command=self.toggle_dark_mode).pack(side="left", expand=True)
    ...
    def autologger(self):
        ...
        notes_window.geometry("500x72")  # Set initial size; width x height

        # Text widget for note-taking
        text = Text(notes_window, wrap=tk.WORD, height=1)  # Set minimal initial height
        ...
        Button(button_frame, text="Submit", command=submit_notes).pack(side=tk.RIGHT, padx=5, pady=5)
        Button(button_frame, text="Cancel", command=notes_window.destroy).pack(side=tk.LEFT, padx=5, pady=5)
        Button(button_frame, text="Insert Divider", command=insert_divider).pack(side=tk.LEFT, padx=5, pady=5)
